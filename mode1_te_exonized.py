...
def prep_overlapped(aln_dir, group, out_group): # OK
    ...
    # TEs_inside = pd.read_csv(str(aln_dir + '/TEs_inside_genes.bed'), sep ='\t', usecols=[0,1,2,3,4,5],names=['scaf_TE', 'start_TE', 'end_TE', 'ID_TE', 'dot_TE', 'strand_TE']).drop_duplicates()
    ...
    if os.stat(str(aln_dir + '/gene_TE_OVERLAPPED-exons.mbed')).st_size > 0:
        # print('There is no TEs overlapping exons with chimeric reads!')
    # else:
    ...
    for row in TEs_overlapping.itertuples():
        ...
        if merging_TE_reads is not None:
            if merging_gene_reads is not None:
                ...
                cov = len(match)
                if cov > 0:
                    # print(str(gene_id) + "\t" + str(strand_gene) + "\t" + str(chr_gene) + ":" + str(s_gene) + "-" + str(e_gene) + "\t" + str(TE_family) + "\t" + str(TE_strd) + "\t" + str(chr_TE) + ":" + str(s_TE) + "-" + str(e_TE) + "\t" + str(cov) + "\t" + str("Overlapped"))
                    with open(str(out_group + '/TE-exonized-' + str(group) + '.tsv'), "a") as te_exon:
...
#the end.
    ...
    # exon_output = str(aln_dir + '/TE-exonized_tmp.tsv')
    # if os.path.exists(exon_output) == True:
    #     if os.stat(str(aln_dir + '/TE-exonized_tmp.tsv')).st_size > 0:
    #         os.rename(aln_dir + '/TE-exonized_tmp.tsv', str('TE-exonized-' + str(group) + '.tsv'))
    #         copy(str(aln_dir + '/TE-exonized-' + str(group) + '.tsv'), out_group)
    #         os.remove('TE-exonized-' + str(group) + '.tsv')
    # else:
    #     print(colored('There is no TE-exonized transcripts!', "red"))


    # if os.stat(str(out_group + "/TE-exonized-" + str(group) + ".tsv")).st_size > 0:


    # copy(str(out_group + "/TE-exonized-" + str(group) + ".tsv"), tmp)
    # That's working full the maximum of threads!
    # processes = []
    # #
    # # for _ in range(int(args.threads)):
    # #
    # for gene in all_gene_IDs:
    #     process = multiprocessing.Process(target = intronic_chimeras, args = (gene,))
    ...
    #     executor.map(intronic_chimeras, (all_gene_IDs,))
    ...
    # processes = []
    # for _ in range(int(args.threads)):
    #     p = multiprocessing.Process(target=intronic_chimeras, args = [all_gene_IDs])
    #     p.start()
    #     processes.append(p)
    #
...
#     with open(str('chim_reads.lst')) as f:
#         all_gene_IDs = f.read().splitlines()
#     f.close

    # processes = []
    # for _ in range(int(args.threads)):
    #     p = map(intronic_chimeras, all_gene_IDs)
    #     p.start()
    #     processes.append(p)
    #
    # for process in processes:
    ...
    #     pool.map(intronic_chimeras, (all_gene_IDs))
    ...
    # all_gene_IDs = pd.read_csv(str('chim_reads.lst')).to_tuple().to_list()
    # print(all_gene_IDs)
    # a, b, c = 1, 2, 3
    #already have it as aln_path, grouped, output


    # partial_sum_four = functools.partial(sum_four, a, b, c)
    # partial_input = partial(intronic_chimeras, aln_path, grouped, output)
    # testing = zip(repeat(str(aln_path)), repeat(str(grouped)), repeat(str(output)), all_gene_IDs)
    ...
    # with Pool(processes=int(args.threads)) as pool:
    #     # testing = list(zip(repeat(str(aln_path)), repeat(str(grouped)), repeat(str(output)), all_gene_IDs))
    #     # print(testing)
    #     # iteract = zip(all_gene_IDs, repeat(aln_path), repeat(grouped), repeat(output))
    #     # print(iteract)
    #     pool.map(intronic_chimeras, list(all_gene_IDs))
    #
    #
    # print(colored("Done!", "green", attrs=['bold']))
...
#     return all_link
    ...
    #     pool.starmap(intronic_chimeras, zip(all_gene_IDs, repeat(aln_path), repeat(grouped), repeat(output)))
    ...
    # pool.starmap(processURL, zip(urlSet, repeat(user), repeat(verboseFlag)))
        ...
        # pool.map(intronic_chimeras, all_gene_IDs, aln_path, grouped, output)
     # feat_one_scale = [pool.map(partial(compute_desc_pool, radius, coords,
                                       # feat, False), coords)]
        # inputing = partial(intronic_chimeras, aln_dir = aln_path, group = grouped, out_group = output)
        # pool.map(inputing, all_gene_IDs)
        # intronic_chimeras, list_args)


# def multicore_process():
#     with open(str('chim_reads.lst')) as f:
#         all_gene_IDs = f.read().splitlines()
#     f.close
...
#     pool.map(inputing, all_gene_IDs)
    ...
    # print(result_list)
        ...
        # pool.map(functools.partial(intronic_chimeras, n=20), range(100000))
