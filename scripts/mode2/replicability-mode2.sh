...
for val in ${StringArray[@]}; do
  ...
  if [[ -s "$PROJECT"/tmp/"$val"_merged.lst ]]; then
    replicated
    total=$(wc -l "$PROJECT"/chimTE-final-chimreads.ct | awk '{print $1}'); echo -e "It has been found "$total" chimeric transcripts in at least $REP replicates!"
    echo -e "The analysis has been finished! Check it out the result in ====> "$PROJECT"/chimTE-final-chimreads.ct\n"
    rm "$PROJECT"/tmp/*"$val"*.lst
  else
    echo -e "There is no chimeric transcripts found in at least "$REP" replicates!!\n"
  fi
done
