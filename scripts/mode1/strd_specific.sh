...
set -e
GENOME_file=$(sed 's|.*/||g' <<< "$GENOME" | sed 's/.fasta//g' | sed 's/.fa//g')
...
if [ -f "$PROJECT"/indexes/"$GENOME_file"_index.1.bt2 ]; then
	...
	samtools view -@ "$THREADS" -bS "$ALN"/accepted_hits.sam > "$ALN"/accepted_hits.bam
	samtools sort -@ "$THREADS" -o "$ALN"/accepted_hits_sorted.bam "$ALN"/accepted_hits.bam
	rm "$ALN"/accepted_hits.bam "$ALN"/accepted_hits.sam
