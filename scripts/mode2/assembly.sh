...
Trinity --seqType fq --SS_lib_type "$STRANDED" --max_memory "$RAM"G --left "$mate1" --right "$mate2" --CPU "$THREADS" --output "$TRINITY_OUT"
echo -e "\nThe transcriptome assembly to sample $sample has been done successfully!\n"; sleep 2
rm -R "$TRINITY_OUT"/chrysalis "$TRINITY_OUT"/insilico_read_normalization "$TRINITY_OUT"/read_partitions/ "$TRINITY_OUT"/jellyfish.kmers.fa "$TRINITY_OUT"/scaffolding_entries.sam "$TRINITY_OUT"/both*
}
...
bowtie2 -x "$TRINITY_OUT"/trinity-assembly -1 "$MATE1" -2 "$MATE2" -S "$TRINITY_OUT"/accepted_hits.sam -p "$THREADS"

#conversion
samtools view -@ "$THREADS" -bS "$TRINITY_OUT"/accepted_hits.sam > "$TRINITY_OUT"/accepted_hits.bam; rm "$TRINITY_OUT"/accepted_hits.sam
bedtools bamtobed -i "$TRINITY_OUT"/accepted_hits.bam > "$TRINITY_OUT"/accepted_hits.bed; rm "$TRINITY_OUT"/accepted_hits.bam
