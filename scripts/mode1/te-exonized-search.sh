...
if [[ -s "$TMP"/genes_TE_INSIDE.txt ]]; then
        ...
        echo -e "There are $genes_TE_DOWN genes with FPKM > $FPKM that have TEs inside genes\nAnalyzing chimeric pairs for TE-exonized transcripts...\n"
        te_exonized
...
#if [ -f "$PROJECT"/"$sample"/TE-exonized_"$sample".ct ]; then
#        cp "$PROJECT"/"$sample"/TE-exonized_"$sample".ct "$PROJECT"/tmp/; else
#        echo -e "There is no TE-exonized transcripts in the "$sample" sample!"
#fi
