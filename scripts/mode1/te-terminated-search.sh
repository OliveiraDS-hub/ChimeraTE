...
bedtools intersect -a "$TE_info"/expressed_TEs.bed -b "$TMP"/genes_DOWN_window.bed -wb -nonamecheck | awk '{print $10}' | sort | uniq > "$TMP"/genes_TE_DOWN.txt
}
...
if [ ! -z $UTR ] ; then
	bedtools intersect -a "$TE_info"/expressed_TEs.bed -b "$GENE_info"/expressed_3utr.bed -wa -wb -nonamecheck | cut -f7-12 | sort | uniq > "$TMP"/genes_TE_3UTR.bed
	while IFS= read -r line
	do
...
if [[ -s "$TMP"/genes_TE_DOWN.txt ]]; then
        ...
        echo -ne "There are $genes_TE_DOWN genes with FPKM > $FPKM with TEs at downstream region\nAnalyzing chimeric pairs for TE-terminated transcripts...\n"
        te_terminated
