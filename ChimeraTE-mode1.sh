...
while IFS= read -r replicates; do
  ...
  echo -e "Running strd_specific.sh - Strand-specific alignment management"
  ./scripts/mode1/strd_specific.sh

  echo -e "Running exp_TEs.sh - Identifying expressed TE insertions -\n"
  ./scripts/mode1/exp_TEs.sh

  echo -e "Running te-initiated-search.sh  - Searching for TE-initiated transcripts -\n"
  ./scripts/mode1/te-initiated-search.sh

  echo -e "Running te-terminated-search.sh  - Searching for TE-terminated transcripts -\n"
  ./scripts/mode1/te-terminated-search.sh

  echo -e "Running te-exonized-search.sh  - Searching for TE-exonized transcripts -\n"
  ./scripts/mode1/te-exonized-search.sh

  #echo -e "\nRunning replicability-smart.sh  - Searching for chimeras identified in at least $REP replicates"
  #./scripts/mode1/replicability-smart.sh
