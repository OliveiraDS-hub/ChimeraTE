...
while IFS= read -r SAMP; do
  ...
  if [[ ! -d "$PROJECT"/"$sample" ]]; then
	mkdir "$PROJECT"/"$sample" "$PROJECT"/"$sample"/alignment "$PROJECT"/"$sample"/chimeric_reads/ "$PROJECT"/"$sample"/plot "$PROJECT"/"$sample"/tmp_dir 2>/dev/null
  fi
...
./scripts/replicability-mode2.sh
