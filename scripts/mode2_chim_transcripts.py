...
def prep_data():
    ...
    pd.read_table(str(aln_dir + '/genes_chim.bed'), sep="\t", usecols=[0], names=['id']).drop_duplicates().to_csv(str(aln_dir + '/gene.lst'), header=None, index=False, sep="\t")
...
def chim_transcript(gene_list):
    ...
    check = None
    chim_genes_complete = pd.read_csv(str(aln_dir + '/genes_chim.bed'), sep="\t", usecols=[0,1,2,3,4,5], names=['transc_id', 'transc_start', 'transc_end', 'transc_read', 'transc_dot', 'transc_strand'])

    TE_chim = pd.read_csv(str(aln_dir + '/TEs_chim.bed'), sep="\t", usecols=[0,1,2,3,4,5], names=['TE_id', 'TE_start', 'TE_end', 'TE_read', 'TE_dot', 'TE_strand'])
    ...
    if check is not None:
        ...
        if cov and int(cov) > 0:
            with open(str(out_group + '/chimTEs_raw.tsv'), "a") as f:
                print(str(gene_id + '\t' + TE_family  + '\t' + cov), file=f)
            f.close()
...
def merging_transc():
    ...
    clock = time()
    print(str(clock) + '\t' + "Merging coverage from different isoforms...")
    chimeras = pd.read_csv(str(out_group + '/chimTEs_raw.tsv'), sep="\t", usecols=[0,1,2], names=['gene_transc', 'TE_id', 'cov'])
    ...
    for gene_id in gene_ids:
        ...
        isoforms = isoforms[:-2]
        with open(str(out_group + '/chimTEs_final.tsv'), "a") as f:
...
def expression():
    ...
    chim_complete = pd.read_csv(str(out_group + '/chimTEs_final.tsv'), sep="\t", usecols=[0,1,2,3], names=['gene_id', 'TE_id', 'cov', 'isoforms'])
    chim_gene_id = pd.read_csv(str(out_group + '/chimTEs_final.tsv'), sep="\t", usecols=[0], names=['col1'])
    chim_gene_id = chim_gene_id['col1'].tolist()
    transcripts_list = pd.read_csv(str(out_group + '/chimTEs_final.tsv'), sep="\t", usecols=[3], names=['transc_id'])
...
def multicore_chimeras():
    ...
    clock = time()
    print(str(clock) + '\t' + "Identifying chimeric transcripts with chimeric reads evidence...")
