...
while IFS= read -r gene_id; do
  ...
  TE_ID=$(grep -F -w "$gene_id" "$PROJECT"/tmp/"$val"_merged.lst | sort -Vr -k5,5 | cut -f3 | head -1)
  ...
  chim_reads=$(grep -F -w "$gene_id" "$PROJECT"/tmp/"$val"_merged.lst | grep -w "$TE_ID" | sort -Vr -k5,5 | cut -f5)
  ...
  if (( $(echo "$avg_reads $COV" | awk '{print ($1 >= $2)}') )); then
    printf "$gene_id""\t""$GENE_strd""\t""$TE_ID""\t""$TE_strd""\t""$TE_pos""\t""$EXON_pos""\t""$avg_reads""\n" >> "$PROJECT"/"$val"_final.ct
  fi
done <<< "$replicated"
}
...
for val in ${StringArray[@]}; do
  ...
  if [[ -s "$PROJECT"/tmp/"$val"_merged.lst ]]; then
    ...
    echo -e "$val transcripts analysis has been finished! Check it out the result in ====> "$PROJECT"/"$val"_final.ct\n"
