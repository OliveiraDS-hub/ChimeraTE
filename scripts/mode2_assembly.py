...
def transcriptome_assembly():
    ...
    if check_file(f"{trinity_out}/Trinity.fasta") == False:
        ...
        print(f"{trinity_out}/trinity_assembly.1.bt2 has been found!"); print(colored("Skipping...\n", "yellow", attrs=['bold']))
    ...
    if check_file(f"{trinity_out}/all_candidates.tsv") == False:
        ...
        print(f"Alignment output has been found!"); print(colored("Skipping...\n", "yellow", attrs=['bold']))
    ...
    if check_file(f"{trinity_out}/all_candidates.tsv") == False:
        ...
        print(f"RepeatMasker output has been found!"); print(colored("Skipping...\n", "yellow", attrs=['bold']))
...
def singleton_crossing():
    ...
    clock = time()
    print(f"{clock}\tIdentifying Trinity transcripts with masked TEs...")
    if check_file(f"{trinity_out}/putative_chimeras.tsv") == False:
        ...
        print(f"Putative chimeras file has been found!"); print(colored("Skipping...\n", "yellow", attrs=['bold']))


    clock = time()
    print(f"{clock}\tPerforming blast to identify transcripts...")
    if check_file(str(f"{trinity_out}/blast-result.tsv")) == False:
        put_chim_isoforms = pd.read_table(f"{trinity_out}/putative_chimeras.tsv", header=None, sep='\t',usecols=[0], names=['isoforms']).to_csv(str(trinity_out + '/putative_isoforms.lst'), header=None, index=False, sep="\t")
    ...
    clock = time()
    print(f"{clock}\tRecovering the best matches...")
    if check_file(str(f"{trinity_out}/{group}_transcriptome_evidence.tsv")) == False:
        blast_result_df = pd.read_table(str(f"{trinity_out}/blast-result.tsv"), header=None, sep='\t', names=['qseqid','sseqid','length','pident','gaps','mismatch','qlen','slen','qstart','qend','sstart','send','evalue','bitscore'])
        ...
        for isoform in isoforms:
            ...
            with open(str(f"{trinity_out}/{group}_transcriptome_evidence.tsv"), 'a') as output:
                print(f"{blast_info}\t{TE_info}", file=output)
            output.close()

        copy(str(f"{trinity_out}/{group}_transcriptome_evidence.tsv"), tmp)
    else:
