...
bedtools intersect -a "$TE_info"/expressed_TEs.bed -b "$TMP"/genes_UP_window.bed -wb -nonamecheck | awk '{print $10}' | sort | uniq > "$TMP"/genes_TE_UP.txt
}
...
do
	...
	echo "$coords_TEs" > "$TMP"/TEs_at_UP-window.txt
		...
		do
			...
			if [ "$strand" == + ]; then
				reads_gene=$(bedtools intersect -a "$GENE_info"/gene_chim_readsFWD.bed -b "$TMP"/tmp_gene_info.bed -nonamecheck)
				...
				reads_gene=$(bedtools intersect -a "$GENE_info"/gene_chim_readsREV.bed -b "$TMP"/tmp_gene_info.bed -nonamecheck)
...
if [ -s "$GENE_info"/expressed_5utr.bed ] ; then
	bedtools intersect -a "$TE_info"/expressed_TEs.bed -b "$GENE_info"/expressed_5utr.bed -wa -wb -nonamecheck | cut -f7-12 | sort | uniq > "$TMP"/genes_TE_5UTR.bed
	while IFS= read -r line
	do
