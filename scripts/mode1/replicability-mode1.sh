...
for val in ${StringArray[@]}; do
  echo -e "############### Checking replicability of $val transcripts ###############\n"
  merging
  if [[ -s "$PROJECT"/tmp/"$val"_merged.lst ]]; then
    replicated
    total=$(wc -l "$PROJECT"/"$val"_final.ct | awk '{print $1}'); echo -e "It has been found "$total" $val transcripts in at least $REP replicates"
